...
namespace Minha_Parte_Biblio
{
    public partial class FrmUserADM : Form
    {
        ...
        ClInnerADMsModelo Modelo_Inner_Users = new ClInnerADMsModelo();
        ...
        void PopularDTVGReserv()
        {
            ...
            try
            {
                data = conexao.obterdados($"SELECT Table_Reservas.CD_Reservas, Table_Livro.Nome_Livro, Table_reservas.DT_reserva, Table_reservas.DT_previsao_devolucao FROM Table_reservas INNER JOIN Table_Livro ON Table_Reservas.CFK_Livro = Table_Livro.Order_Livro where Table_Reservas.CFK_User = {Modelo_Inner_Users.Index}");
                dataGridView.Columns.Clear();
                // Definindo a fonte de dados do DataGridView
                dataGridView.DataSource = data;
                StyleDataGridView();
        ...
        private void dataGridView_Click(object sender, EventArgs e)
        {
            ...
            if (dvg.Rows.Count > 0 && dvg.CurrentCell.RowIndex >= 0)
            {
                ...
                Modelo_Inner_Users.Index = Convert.ToInt32(row.Cells["Index"].Value);
                Modelo_Inner_Users.ID_Aluno = row.Cells["ID"].Value.ToString();
                Modelo_Inner_Users.NomeCompleto = row.Cells["Nome Completo"].Value.ToString();
                Modelo_Inner_Users.UserName = row.Cells["NameUser"].Value.ToString();
                Modelo_Inner_Users.Cargo = row.Cells["Cargo"].Value.ToString();
                Modelo_Inner_Users.Ano = row.Cells["Ano"].Value.ToString();
                Modelo_Inner_Users.Unidade = row.Cells["Unidade"].Value.ToString();
                Modelo_Inner_Users.NumbReservas = Convert.ToInt32(row.Cells["Numb Reservas"].Value);
                Modelo_Inner_Users.NumbHistorico = Convert.ToInt32(row.Cells["Numb Histórico"].Value);
        ...
        {
            int exclude = conexao.excluir(0, $"DELETE FROM Table_User WHERE CD_User = {Modelo_Inner_Users.Index};", "@CD_User");
            if (exclude > 0)
            {
